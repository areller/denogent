...
describeE2E('tasks.test.ts', t => {
    t.test(`'denogent tasks' should return list of tasks (no json)`, async () => {
        await createBuildInTempDir(async temp => {
            let lines: string[] = [];
            const [success, _] = await runCommand([...denogent, 'tasks', '--file', 'build.ts'], line => {
    ...
    t.test(`'denogent tasks' should return list of tasks (json)`, async () => {
        await createBuildInTempDir(async temp => {
            let lines: { task: Task }[] = [];
            const [success, _] = await runCommand([...denogent, 'tasks', '--file', 'build.ts', '--json'], line => {
            ...
            assertEquals(lines.filter(x => x.task.name == 'taskA').map(x => x.task), [{
                name: 'taskA',
                conditions: [],
                dependencies: [],
                dependents: ['taskB'],
            ...
            assertEquals(lines.filter(x => x.task.name == 'taskB').map(x => x.task), [{
                name: 'taskB',
                conditions: [],
                dependencies: ['taskA'],
                dependents: [],

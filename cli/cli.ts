...
): Promise<[Runtime, Graph | undefined]> {
  const graph = buildContext !== undefined
    ? createGraph(buildContext.targetTasks)
    : undefined;
  let runtime: Runtime;
  if (
    buildContext !== undefined && args["runtime"] && args["runtime"] != "local"
  ) {
    const ciArray = buildContext.ciIntegrations.filter((c) =>
      c.type == args["runtime"]
    );
...
export async function createCLI(buildContext?: BuildContext) {
  ...
  await new Command()
    ...
    .option(
      ...
      { global: true, default: defaultBuildFile },
    )
    .option(
      "--json [:boolean]",
      "Emit logs as json.",

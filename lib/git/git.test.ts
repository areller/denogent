...
describe("git.test.ts", (t) => {
  [
    [false, "non-repo"] as [boolean, string],
    [true, "simple-repo"] as [boolean, string],
  ].forEach((asset: [boolean, string]) => {
    ...
    t.test("getBranch should return the current branch", async () => {
      await copyDirToTemp(
        stdPath.join(assetsPath, "simple-repo"),
        async (temp) => {
    ...
    t.test("describe should return closest tag and distance from that tag", async () => {
      await copyDirToTemp(
        stdPath.join(assetsPath, "tagged-repo"),
        async (temp) => {
          const describe = await git.describe({
            path: temp,
            logger: false,

...
describe('runtime.test.ts', t => {
    t.test('command should run a command', async () => {
        await emptyTempDir(async temp => {
            ...
            let lines: string[] = [];
            const [success, output] = await runtime.command({ path: temp, cmd: 'ls', logger: mockDebugLogger(line => lines.push(line)) });
    ...
    [false, true, undefined].forEach(throws => {
        t.test('command ' + (throws === true || throws === undefined ? 'should' : `shouldn't`) + ' throw an exception when fails', async () => {
            await copyDirToTemp(assetsPath, async temp => {
                try {
                    const [status, _] = await runtime.command({ path: temp, cmd: ['deno', 'run', '--no-check', '-A', 'fail.ts'], logger: false, throwOnFailure: throws });

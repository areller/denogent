...
export class Folder {
  ...
  async listFiles(): Promise<File[]> {
    var files: File[] = [];
    for await (
      const entry of stdFs.walk(
        this._path,
        { maxDepth: 1, includeFiles: true, includeDirs: false },
      )
    ) {
      if (entry.isFile) {
  ...
  async listFolders(): Promise<Folder[]> {
    ...
    for await (
      ...
      if (
        entry.isDirectory &&
        stdPath.normalize(entry.path) != stdPath.normalize(this._path)
      ) {
  ...
  async getFolder(path: string | string[]): Promise<Folder | undefined> {
    const folderPath = stdPath.join(
      this._path,
      path instanceof Array ? stdPath.join(...path) : path,
    );

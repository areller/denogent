    ...
    return (
      ...
      if (meta !== undefined) {
        ...
        if (attrs.type == "started") {
          ...
          ["finishedSuccessfully", "failedCondition", "failed"].indexOf(
            attrs.type,
          ) != -1
        ) {
...
export class GitHubActions implements CIIntegration {
  ...
  async clean(args: CleanArgs): Promise<void> {
    const workflowsPath = args.path === undefined
      ? stdPath.join(".github", "workflows")
      : stdPath.join(args.path, ".github", "workflows");
        ...
        [this.image]: {
          ...
          steps: [
            ...
            ...this.buildUses(args),
            {
              name: "run build",
              run:
                `deno run -A -q --unstable ${args.buildFile} run --serial --runtime gh-actions`, // currently relies on unstable API + GitHub Actions only supports serial execution at the moment
              env: {
                ...runEnv,
      ...
      if (dockerServices !== undefined) {
        for (const dockerService of Object.values(dockerServices)) {
          services[dockerService.name] = {
            ...
            ports: this.dockerImage === undefined
              ? dockerService.ports.map((p) => `${p}:${p}`)
              : [],
          };
          ...
          if (dockerService.ports.length > 0) {
            runEnv[`${dockerService.name.toUpperCase()}_PORT`] = dockerService
              .ports[0].toString();
            runEnv[`${dockerService.name.toUpperCase()}_PORTS`] = dockerService
              .ports.join(";");
          }
        }
      }

...
export class DockerClient {
  ...
  async subcmd(args: DockerClientSubCommandArgs): Promise<string> {
    let [_, output] = await this.runDocker(
      args,
    ...
    return {
      ...
      enrich: (t) => {
        ...
        if (services[args.name]) {
          throw new Error(
            `Task '${t.name}' already has a service '${args.name}' in the 'docker-services' property.`,
          );
        }
    ...
    return {
      ...
      enrich: (t) => {
        if (t.properties["docker-image"]) {
          throw new Error(
            `Task '${t.name}' already has a 'docker-image' property.`,
          );
        }

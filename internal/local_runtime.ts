...
export class LocalRuntime implements Runtime {
  ...
  async launchDockerServices(): Promise<void> {
    ...
    for (const taskName of this.graph!.taskNames) {
      ...
      const dockerServices = task.properties["docker-services"] as {
        [name: string]: Service;
      } | undefined;
    ...
    if (
      !(await runCommand(["docker", "--version"], undefined, undefined, false))[
        0
      ]
    ) {
    ...
    for (const service of Object.values(services)) {
      ...
      if (service.ports.length > 0) {
        Deno.env.set(
          `${service.name.toUpperCase()}_PORT`,
          service.ports[0].toString(),
        );
        Deno.env.set(
          `${service.name.toUpperCase()}_PORTS`,
          service.ports.join(";"),
        );
      }
    }

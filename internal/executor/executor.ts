...
export class Execution {
  ...
  private spawnCollectionOfTasks(
    tasks: string[],
    conditionFn?: (task: Task) => boolean,
  ): void {
  ...
  private async runTask(task: Task): Promise<void> {
    try {
      ...
      try {
        // running task
        const execTask = task.exec !== undefined
  ...
  private finishTask(
    ...
    if (failedCondition !== undefined) {
      this._tasksTracker[task.name]!.failedCondition = [
        failedCondition,
        task.conditions[failedCondition].toString(),
      ];
    }
    ...
    return {
      logger: {
        ...
        error: (msg: string | Error, meta?: unknown) =>
          this.writeLogToTask(task, "error", msg, meta),
      },
    };
  }
}

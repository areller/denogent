...
describe('graph.test.ts', t => {
    ...
    t.test('getTasksByLevel should return a map from level to tasks in that level', () => {
        ...
        assertEquals(levels[0].map(t => t.name), ['a', 'a1']);
        assertEquals(levels[1].map(t => t.name), ['b', 'c']);
        assertEquals(levels[2].map(t => t.name), ['b1', 'd']);
        assertEquals(levels[3].map(t => t.name), ['e']);
    });
});
